...
# %%
# dispersion relation
g = grid_log(10.0, 1.04, 400)
grid = [-reverse(g); 0; g]
# grid = range(-10; stop = 10, length = 801)
n = 1000
ϵ = 6.2
μ = ϵ / 2
x = 0.5
...
G_loc2, Σ2 = cpa_loop(dispersion, grid; x, ϵ);
...
lines!(ax, W, -imag(evaluate_lorentzian(Σ1, W, δ)); label = "resolvent");
lines!(ax, grid, -imag(Σ2); label = "finite broadening");
axislegend(ax; position = :lt);
